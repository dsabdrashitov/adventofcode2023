...
def solve(f):
    ...
    for i in range(len(field[0])):
        answer = max(answer, heat((D_D, (0, i)), field))
        answer = max(answer, heat((D_U, (len(field) - 1, i)), field))
    for i in range(len(field)):
        answer = max(answer, heat((D_R, (i, 0)), field))
        answer = max(answer, heat((D_L, (i, len(field[0]) - 1)), field))
    return answer
